...
export const FindProductWithTime = expressAsyncHandler(async (req, res) => {
    if (start && end) {
        const Order = await OrderModel.find({
            createdAt: {
                $gte: new Date(start),
        ...
        if (status) {
            const findList = Order?.filter((e) => e.status == status);
            res.send(findList);
        } else {
            if (Order) {
                res.send(Order);
                ...
                res.status(400).send({
                    message: "no product in a time search",
                });
            }
        }
    } else {
...
export const FindInfoOder = expressAsyncHandler(async (req, res) => {
    ...
    if (start && end) {
        ...
        if (product && order && user) {
            res.send({
                product: { list: [...product], length: product?.length ?? 0 },
                order: { list: [...order], length: order?.length ?? 0 },
                user: { list: [...user], length: user?.length ?? 0 },
            });
        } else {
            res.status(400).send({
